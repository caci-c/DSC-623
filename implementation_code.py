...
df_tables = pd.read_sql_query("SELECT name FROM sqlite_master WHERE type='table';", db_connect)
for table in df_tables['name']:
    df_columns = pd.read_sql_query(f"PRAGMA table_info('{table}');", db_connect)
